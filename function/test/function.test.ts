import { authHandler } from "../src/authorizer.lambda";
...
const tokens = {
  invalidForKey: {
    payload: { foo: "bar" },
    token: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.e30.qPs0ENhI_xXsVP9PKgHR47AB6WShFWscA9xrDN0h1Bc",
  },
  validEmpty: {
    payload: { foo: "bar" },
    token: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJmb28iOiJiYXIifQ.s94EkJ9nxp6LVSUNtIHJPuIRE_2flT_fQclxXPMXSH4",
  },
  validExpired: {
    payload: { foo: "bar" },
    token:
  ...
  validNotExpiredValidationIssues: {
    payload: { foo: "42" },
    token:
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJmb28iOiI0MiIsImV4cCI6MTk2ODE5MDM5N30.G2AYPQl3VPlSj-GqVuHVR_xb-pFClNN1ox2JsIijiNg",
  },
};
...
describe("Token authorizer returns policyDocument", function () {
  ...
  it("verifies exception if no jwt token is provided", async () => {
    ...
    await expect(authHandler(eventPayload())).rejects.toThrow("MISSING_AUTHORIZATION_TOKEN");
  });

  it("verifies INVALID_PROCESS_ENV error if NO secret or jwks uri are provided", async () => {
    await expect(authHandler(eventPayload("foo"))).rejects.toThrow("MISSING_PROCESS_ENV_VARIABLES");
  });

  it("verifies exception if jwks uri is set but no jwks kid is provided", async () => {
    process.env.TOKEN_AUTHORIZER_JWKS_URI = jwksUri;

    await expect(authHandler(eventPayload("foo"))).rejects.toThrow("MISSING_JWKS_KID");
  });
  ...
  it("verifies exception if jwks uri is invalid", async () => {
    ...
    await expect(authHandler(eventPayload("foo"))).rejects.toThrow("JWKS_ERROR");
  });
  ...
  it("verifies exception if jwks has invalid response", async () => {
    ...
    await expect(authHandler(eventPayload("foo"))).rejects.toThrow("JWKS_SYNTAXERROR");
  });
  ...
  it("verifies exception if jwks key is is not found", async () => {
    ...
    await expect(authHandler(eventPayload("foo"))).rejects.toThrow("JWKS_SIGNINGKEYNOTFOUNDERROR");
  });
  ...
  it("verifies that a token wich is not matching to the provided key from jwks returns unauthorized", async () => {
    ...
    const response = await authHandler(eventPayload(tokens.invalidForKey.token));
  ...
  it("verifies that a token wich which can be verfified against a asymmetric secret returns allowed", async () => {
    ...
    const response = await authHandler(eventPayload(token));
  ...
  it("verifies allow policy and context data on valid token with asymetric key set", async () => {
    ...
    const response = await authHandler(eventPayload(tokens.validEmpty.token));
    expect(response.policyDocument.Statement[0].Effect).toEqual("Allow");
  ...
  it("verifies deny policy on expired token", async () => {
    ...
    const response = await authHandler(eventPayload(tokens.validExpired.token));
    expect(response.policyDocument.Statement.length).toEqual(1);
    expect(response.policyDocument.Statement[0].Effect).toEqual("Deny");
  ...
  it("verifies deny policy and context data on valid token with payload validation errors", async () => {
    ...
    const response = await authHandler(eventPayload(tokens.validNotExpiredValidationIssues.token));
    expect(response.policyDocument.Statement[0].Effect).toEqual("Deny");
    expect(response.context!.errorType).toEqual("JWT_PAYLOAD_VALIDATION_ERROR");
    expect(response.context!.errorMessage).toEqual(
      '[{"instancePath":"/foo","schemaPath":"/properties/foo/enum","keyword":"enum","params":{"allowedValues":["bar"]},"message":"must be equal to one of the allowed values"}]'
    );
  });
});
